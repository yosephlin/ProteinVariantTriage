...
def run_round_pipeline(
    ...
    selector_cmd = [
        ...
        "--binding-challenger-frac", str(float(clamp(float(args.binding_challenger_frac), 0.0, 1.0))),
        ...
        "--chemistry-challenger-frac", str(float(clamp(float(args.chemistry_challenger_frac), 0.0, 1.0))),
        "--chemistry-challenger-min", str(max(0, int(args.chemistry_challenger_min))),
        ...
        "--min-chemistry-challenger-selected", str(max(0, int(args.min_chemistry_challenger_selected))),
    ]
    selector_cmd.append(
...
def main() -> int:
    ...
    ap.add_argument("--binding-challenger-frac", type=float, default=0.0)
    ap.add_argument("--binding-challenger-min", type=int, default=0)
    ...
    ap.add_argument("--binding-context-blend-weight-binary", type=float, default=0.60)
    ap.add_argument("--binding-context-blend-weight-ternary", type=float, default=0.40)
    ...
    if bool(args.with_janus_final):
        run_final_janus(
            outdir=outdir,
            py=py,
            rounds=[int(h.get("round")) for h in history if h.get("round") is not None],
            args=args,
        )

    print("[recursive] flow complete")
    print(f"[recursive] summary: {summary_path}")
    print(f"[recursive] final panel: {panel_path(outdir=outdir, round_id=int(final_round))}")
    if bool(args.with_janus_final):
