...
def main() -> int:
    ap = argparse.ArgumentParser(description="Final Janus scoring over merged SWARM panels across rounds.")
    ...
    for rnd in rounds:
        ...
        with panel_path.open() as fh:
            ...
            for raw_row in reader:
                ...
                if (new_score > old_score) or (new_score == old_score and int(row["round_source"]) > int(old.get("round_source", "-1"))):
                    rows_by_variant[vid] = row
                    dedup_replaced += 1
    ...
    final_panel_summary = Path(args.final_dir).resolve() / "swarm_final_panel_summary.json" if args.final_dir else final_swarm_panel_summary_path(outdir)
